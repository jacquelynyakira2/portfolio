...
const Metamorphosis = ({ size = 420 }: MetamorphosisProps) => {
  ...
  useEffect(() => {
    ...
    const forms = [
      ...
      (u: number, v: number, t: number) => {
        ...
        const z = r * Math.cos(phi) + 20 * Math.sin(theta * 5 + phi * 3);
    ...
    const getCurrentForm = (u: number, v: number, t: number) => {
      ...
      let blend: number;
      if (rawBlend < pauseTime) blend = 0;
      else if (rawBlend > 1 - pauseTime) blend = 1;
      else {
    ...
    const animate = () => {
      ...
      const rotateX = Math.sin(time * rotateSpeed) * 0.5; // eslint-disable-line @typescript-eslint/no-unused-vars
      const rotateY = Math.cos(time * rotateSpeed * 0.7) * 0.3; // eslint-disable-line @typescript-eslint/no-unused-vars
      ...
      for (let i = 0; i < numLines; i++) {
        ...
        for (let j = 0; j <= lineSegments; j++) {
          ...
          if (j === 0) {
            ...
            if (pointVisible && lastPointVisible) ctx.lineTo(projX, projY);
            else if (pointVisible && !lastPointVisible) ctx.moveTo(projX, projY);
          }
